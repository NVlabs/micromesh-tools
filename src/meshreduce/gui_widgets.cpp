...
void GUIApplication::_input_widgets()
{
	...
	if (ImGui::Button("Load...", ImVec2(w1, 0))) {
		nfdchar_t* file_path = NULL;
		nfdresult_t result = NFD_OpenDialog("obj,stl,gltf", NULL, &file_path);
...
void GUIApplication::_layer_widgets()
{
	...
	ImVec2 cell_padding(4, 2);

	ImGui::PushStyleVar(ImGuiStyleVar_CellPadding, cell_padding);
	ImGui::BeginTable("view_options", 3);
	...
	if (s.proxy.F.rows() > 0) {
		...
		std::snprintf(strbuf, 64, "%s F", fn_proxy.c_str());
		ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() - ImGui::CalcTextSize(strbuf).x);
		ImGui::Text("%s", strbuf);
		ImGui::TableSetColumnIndex(2);
	...
	std::snprintf(strbuf, 64, "%s F", fn_base.c_str());
	ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() - ImGui::CalcTextSize(strbuf).x);
	ImGui::Text("%s", strbuf);
	ImGui::EndDisabled();
	ImGui::TableSetColumnIndex(2);
	...
	std::snprintf(strbuf, 64, "%s F", fn_micro.c_str());
	ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() - ImGui::CalcTextSize(strbuf).x);
	ImGui::Text("%s", strbuf);
	ImGui::TableSetColumnIndex(2);
...
void GUIApplication::_base_mesh_generation_widgets()
{
	...
	ImGui::SameLine();
	HelpMarker("As a fraction of the average input edge length");
	ImGui::EndDisabled();
...
void GUIApplication::_base_mesh_optimization_widgets()
{
	...
	ImGui::SameLine(gui.layout.tweak_marker_x);
...
void GUIApplication::_micromesh_generation_widgets()
{
	...
	ImGui::BeginDisabled(gui.tessellate.current_mode == 0);
	...
	bool displacement_was_enabled = gui.displace.enabled;

	ImGui::Indent(gui.layout.widget_indentation);

	ImGui::PushStyleVar(ImGuiStyleVar_SelectableTextAlign, ImVec2(0.5f, 0.5f));
	ImGui::BeginTable("Displacement Mode", 1, ImGuiTableFlags_Resizable | ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_Borders);
	ImGui::TableNextRow();
	ImGui::TableSetColumnIndex(0);
	if (ImGui::Selectable("Ray-cast (scalar)", gui.displace.enabled == true, 0, ImVec2(0, 20))) {
		gui.displace.enabled = !gui.displace.enabled; // toggle
	}
	ImGui::EndTable();
	ImGui::PopStyleVar();
	...
	if (gui.micromesh_exists) {
		bool needs_displacement =
			(!s.micromesh.is_displaced() && gui.displace.enabled) // displacement enabled and micromesh is not displaced
			|| (!displacement_was_enabled && gui.displace.enabled) // toggled displace button and is currently active
			|| gui.directions.changed;
...
void GUIApplication::_extra_overlay()
{
	...
	ImGui::Begin("Misc. controls", nullptr, ImGuiWindowFlags_NoResize);
	...
	ImGui::InputFloat("Input Qual. Thresh.", &quality.max_hi_quality, 0.001f, 0.01f, "%.4f");
	ImGui::Separator();

	redraw |= ImGui::Checkbox("Draw Micromesh Wireframe", &ro.draw_umesh_wire);
